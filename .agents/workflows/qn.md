...
- `staff_members` là bảng nhân viên chuẩn; tra cứu tên theo `staff_members.id → name`. Quyền trực tiếp lấy từ `modules` và `actions`; mọi kiểm soát quan trọng vẫn phải được server/RLS xác nhận.
