...
// ── Providers ─────────────────────────────────────────────────────────────────
final _staffListProvider = FutureProvider.autoDispose<List<StaffMember>>((
  ref,
) async {
...
final _shiftsProvider = FutureProvider.autoDispose<List<ShiftRecord>>((
  ref,
) async {
  final session = ref.watch(sessionProvider);
  if (session?.storeId == null) return [];
...
final _shiftConfigsProvider = FutureProvider.autoDispose<List<ShiftConfig>>((
  ref,
) async {
  final session = ref.watch(sessionProvider);
  if (session?.storeId == null) return [];
...
final _openShiftProvider = FutureProvider.autoDispose<Map<String, dynamic>?>((
  ref,
) async {
  final session = ref.watch(sessionProvider);
                ...
                children: [
                  ...
                  Consumer(
                    builder: (_, ref, __) {
                      ...
                      final active =
                          list.value?.where((s) => s.isClockedIn).length ?? 0;
                      return Text(
                        '$count thành viên • $active đang làm ca',
                        style: const TextStyle(
    ...
    return async.when(
      ...
      data: (all) {
        final unassignedCount = all
            .where((m) => _isUnassignedStaff(m, storeRoles))
            .length;
...
class _StaffDetailSheetState extends ConsumerState<_StaffDetailSheet>
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          // Quán
                          Text(
                            '· ${ref.read(sessionProvider)?.storeName ?? ''}',
                            style: const TextStyle(
                              fontSize: 11,
                              color: _kMuted,
                            ),
  ...
  Future<void> _saveProfile() async {
    ...
    await StaffService.updateProfile(
      ...
      baseSalary:
          double.tryParse(_baseSalaryCtrl.text.replaceAll(',', '')) ??
          widget.member.baseSalary,
      hourlyRate:
          double.tryParse(_hourlyRateCtrl.text.replaceAll(',', '')) ??
          widget.member.hourlyRate,
        ...
        return StatefulBuilder(
          builder: (dialogCtx, setDialogState) {
            return AlertDialog(
              ...
              content: Text(
                'Nhân viên $targetName ($targetUserId) sẽ không còn truy cập quán này.',
              ),
              actions: [
                TextButton(
                  ...
                  onPressed: isDeleting
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('✅ Đã xoá nhân viên $targetName'),
                                behavior: SnackBarBehavior.floating,
                              ),
                            );
                          } catch (e) {
              ...
              : SliverPadding(
                  ...
                  sliver: SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (_, i) =>
                          _ShiftCard(shift: shifts[i], showName: isManager),
...
class _ClockCardState extends ConsumerState<_ClockCard> {
  ...
  String _formatElapsed(Duration d) {
    ...
    if (h > 0)
      return '${h}g ${m.toString().padLeft(2, '0')}p ${s.toString().padLeft(2, '0')}s';
  ...
  Future<void> _clockOut(String shiftId) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ Ca làm việc đã kết thúc'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    }
  ...
  Widget _buildShell({required Widget child, bool isClockedIn = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: (isClockedIn ? const Color(0xFF065F46) : _kNavy).withValues(
              alpha: 0.4,
            ),
            blurRadius: 16,
...
class _ShiftCardState extends ConsumerState<_ShiftCard> {
  ...
  Future<void> _editShift(BuildContext ctx) async {
    ...
    ref.invalidate(_shiftsProvider);
    ref.invalidate(
      _monthlyShiftsProvider(DateTime(s.clockIn.year, s.clockIn.month)),
    );
    ScaffoldMessenger.of(ctx).showSnackBar(
      const SnackBar(
        content: Text('✅ Đã cập nhật giờ ca'),
        behavior: SnackBarBehavior.floating,
      ),
    );
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${fmt.format(s.clockIn)} → ${s.clockOut != null ? fmt.format(s.clockOut!) : "Đang làm"}',
                  style: const TextStyle(
                    fontSize: 13,
                    fontWeight: FontWeight.w600,
                    color: _kNavy,
...
class _PermissionsTab extends ConsumerWidget {
  ...
  static const _modLabels = <String, String>{
    ...
    'tinhluong': 'Lương',
    'kay_ops': 'Vận hành', // ✅ FIX: thêm 2 module còn thiếu
  };
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 6),
                const Text(
                  'Nhấn "Quản lý vai trò" để tạo mới',
                  style: TextStyle(fontSize: 12, color: _kMuted),
                ),
              ],
            ),
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'Nhấn vào vai trò để chỉnh module',
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              '${r.modules.length}/${_modLabels.length} module',
                                              style: TextStyle(
                                                fontSize: 12,
                                                fontWeight: FontWeight.w700,
                ...
                const Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Nhân viên ca nào thấy việc ca đó',
                  ...
                  data: (shifts) {
                    if (shifts.isEmpty) {
                      return Container(
                        ...
                        child: const Text(
                          'Chưa có ca nào. Nhấn "Thêm ca" để tạo.',
                          style: TextStyle(fontSize: 13, color: _kMuted),
                        ),
                      );
                    }
...
class _ShiftConfigCard extends StatefulWidget {
  ...
  @override
  State<_ShiftConfigCard> createState() => _ShiftConfigCardState();
}
...
class _ActionPermsSection extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_ActionPermsSection> createState() =>
      _ActionPermsSectionState();
}
...
class _ActionPermsSectionState extends ConsumerState<_ActionPermsSection> {
  ...
  Future<void> _loadPermissions(String roleName) async {
    ...
    setState(() => _loading = true);
    final perms = await StaffService.getActionPermissions(
      widget.storeId,
      roleName,
    );
    if (mounted)
      setState(() {
        _enabledActions = perms;
        _loading = false;
      });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final allowedModules = selectedRoleObj.modules
    ...
    for (final key in kAllActions) {
      ...
      final isModuleAllowed =
          allowedModules.contains(modulePrefix) ||
                ...
                const Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Kiểm soát điều nhân viên được làm',
                  ...
                  Container(
                    ...
                    child: const Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Chưa có vai trò nào. Tạo vai trò trước.',
                            style: TextStyle(fontSize: 12, color: _kMuted),
                          ),
                        ),
                      ],
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Switch.adaptive(
                                            ...
                                            activeTrackColor: _kNavy.withValues(
                                              alpha: 0.4,
                                            ),
                                            onChanged: (_) {
                                              HapticFeedback.selectionClick();
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(
                        _saving
                            ? 'Đang lưu...'
                            : 'Lưu quyền cho "$_selectedRoleName"',
                        style: const TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w700,
                        ),
...
class _ShiftStaffPanel extends StatefulWidget {
  ...
  @override
  State<_ShiftStaffPanel> createState() => _ShiftStaffPanelState();
}
...
class _ShiftStaffPanelState extends State<_ShiftStaffPanel> {
  ...
  List<StaffMember> get _unassigned => _allStaff
      .where((s) => s.shiftConfigId != widget.shift.id && !s.isOwner)
      .toList();
    ...
    return Column(
      ...
      children: [
        ...
        if (_assigned.isEmpty)
          Text(
            'Chưa có nhân viên nào. Nhấn "Thêm" để gán.',
            style: const TextStyle(fontSize: 12, color: _kMuted),
          )
        else
          Wrap(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const Text(
            'Chọn nhân viên chưa được phân ca',
            style: TextStyle(fontSize: 12, color: _kMuted),
          ),
          const SizedBox(height: 16),
...
class _ShiftFormSheetState extends State<_ShiftFormSheet> {
  ...
  Future<void> _save() async {
    ...
    if (widget.storeId.isEmpty) {
      setState(
        () => _errorMsg = 'Lỗi: storeId rỗng — thử đăng xuất và đăng nhập lại',
      );
      return;
    }
    setState(() {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              widget.existing == null ? 'Thêm ca làm việc' : 'Sửa ca',
              style: const TextStyle(
                fontSize: 17,
                fontWeight: FontWeight.w900,
                color: _kNavy,
            ...
            const SizedBox(height: 6),
            const Text(
              'Chọn nhanh hoặc tự nhập tên ca',
              style: TextStyle(fontSize: 12, color: _kMuted),
            ),
            const SizedBox(height: 12),
            ...
            TextField(
              controller: _nameCtrl,
              onChanged: (_) =>
                  setState(() {}), // để chips cập nhật selected state
              decoration: InputDecoration(
                labelText: 'Tên ca',
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _saving
                    ...
                    : Text(
                        widget.existing == null ? '✓ Tạo ca' : '✓ Lưu thay đổi',
                        style: const TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.w800,
                        ),
...
class _StaffRightPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (final s in staff) {
      final name = s.isOwner
          ? 'Chủ quán'
          : _resolveRole(s.role, storeRoles).name;
