          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                _DetailRow(
                  'Lương cơ bản (${_modeLabel(record.salaryMode)})',
                  record.regularPay,
                  color: Colors.green,
                ),
                ...
                if (record.deductionLate > 0)
                  _DetailRow(
                    'Đi muộn (${record.lateCount} lần)',
                    -record.deductionLate,
                    color: Colors.red,
                  ),
                if (record.deductionAbsent > 0)
                  _DetailRow(
                    'Nghỉ không phép (${record.absentDays} ngày)',
                    -record.deductionAbsent,
                    color: Colors.red,
                  ),
                if (record.deductionManual > 0)
                  _DetailRow(
                    'Khấu trừ khác',
                    -record.deductionManual,
                    color: Colors.red,
                  ),
                  ...
                  error: (e, _) => Center(
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          const Text(
                            'Lỗi tải dữ liệu khoản phát sinh',
                          ...
                          SizedBox(
                            ...
                            child: OutlinedButton(
                              onPressed: () => ref.invalidate(
                                payrollItemsProvider(record.id),
                              ),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _PayslipStatusBadge(status: record.paymentStatus),
              ),
              Expanded(
                child: _PayslipStat('Chế độ', _modeLabel(record.salaryMode)),
              ),
            ],
          ),
          const SizedBox(height: 16),
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
            child: Text(label, style: style ?? const TextStyle(fontSize: 14)),
          ),
          Text(
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 4),
                    const Text(
                      'Nhấn để xem chi tiết ca làm việc',
...
class _StatCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      Text(
        label,
        style: const TextStyle(fontSize: 9, color: Color(0xFF9E9085)),
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (context.mounted) {
        setState(() {
          _loading = false;
          _error = 'Lỗi tải chi tiết ca làm việc';
        });
      }
    }
  }
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${widget.periodFrom} → ${widget.periodTo}',
                          style: const TextStyle(
                            fontSize: 12,
                            color: Color(0xFF9E9085),
                          ),
            ...
            Expanded(
              child: _loading
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        final s = _shifts[i];
                        final ci = DateTime.parse(
                          s['clock_in'] as String,
                        ).toLocal();
                        final co = s['clock_out'] != null
                            ? DateTime.parse(s['clock_out'] as String).toLocal()
                            : null;
                        final hours = co != null
                            ? co.difference(ci).inMinutes / 60.0
                            : null;
                        final isLate = (s['is_late'] as bool?) ?? false;
                        String fmt2(int v) => v.toString().padLeft(2, '0');
                        final timeStr =
                            '${fmt2(ci.hour)}:${fmt2(ci.minute)} → '
                            '${co != null ? '${fmt2(co.hour)}:${fmt2(co.minute)}' : '—'}';
                        final dateStr =
                            '${fmt2(ci.day)}/${fmt2(ci.month)}/${ci.year}';
                        return ListTile(
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: CircleAvatar(
        radius: 20,
        backgroundColor:
            (isBonus ? const Color(0xFF16A34A) : const Color(0xFFDC2626))
                .withValues(alpha: 0.1),
        child: Icon(
          isBonus ? Icons.add : Icons.remove,
          color: isBonus ? const Color(0xFF16A34A) : const Color(0xFFDC2626),
          size: 18,
        ),
      ),
      title: Text(
...
class _DisputeTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasReply =
        dispute['reply'] != null && (dispute['reply'] as String).isNotEmpty;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isOpen ? const Color(0xFFFFF3F3) : const Color(0xFFF5F0EA),
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              Text(
                _formatDate(dispute['created_at'] as String? ?? ''),
  ...
  void _dismiss(BuildContext context) async {
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Không thể bỏ qua. Vui lòng thử lại.')),
        );
      }
    }
  }
...
class _ExportPdfSheetState extends State<_ExportPdfSheet> {
  ...
  Future<PayslipStoreInfo> _loadStoreInfo() async {
    ...
    const customerDefaults = ['Cảm ơn quý khách!', 'Cảm ơn quý khách', ''];
    final footer = customerDefaults.contains(billFooter.trim())
        ? 'Cảm ơn bạn vì sự cố gắng trong tháng qua!'
        : billFooter;
    return PayslipStoreInfo(
      name: (info['store_name'] ?? '').isNotEmpty
          ? info['store_name']!
          : 'Quán Nhỏ',
      phone: phone,
      address: address,
  ...
  Future<void> _export({required bool isA4, required bool preview}) async {
    ...
    try {
      ...
      final freshRecord =
          await TinhLuongRepository.fetchRecordById(widget.record.id) ??
      ...
      // Tên file dùng chung
      final fileName =
          'PhieuLuong_${widget.record.staffName.replaceAll(' ', '_')}'
      ...
      if (preview) {
        ...
        await SharePlus.instance.share(
          ShareParams(
            ...
            text:
                'Phiếu lương: ${widget.record.staffName} — ${widget.period.name}',
          ),
        );
        if (mounted) {
          Navigator.pop(context);
      ...
      if (mounted) {
        setState(() {
          _error = 'Không thể tạo phiếu lương. Vui lòng thử lại.';
          _loading = false;
        });
      }
    }
