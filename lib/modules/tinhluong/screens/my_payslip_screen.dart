...
final myPayslipsProvider = FutureProvider<List<_MyPayslipEntry>>((ref) async {
  ...
  final rows = await db.from('payroll_records')
  ...
  if (rows.isNotEmpty) {
    return rows.map<_MyPayslipEntry>((r) {
  ...
  try {
    final accountRow = await db.from('user_accounts')
        .select('display_name')
        .eq('id', userId)
        .maybeSingle();
    final staffName = accountRow?['display_name'] as String?;
    if (staffName != null && staffName.isNotEmpty) {
      final fallbackRows = await db.from('payroll_records')
          .select('*, payroll_periods!inner(name, from_date, to_date, status)')
          .eq('store_id', storeId)
          .ilike('staff_name', '%$staffName%')
          .order('created_at', ascending: false);
      if (fallbackRows.isNotEmpty) {
        return fallbackRows.map<_MyPayslipEntry>((r) {
          final p = r['payroll_periods'] as Map<String, dynamic>;
          return _MyPayslipEntry(
            record: PayrollRecordModel.fromMap(r),
            periodName: p['name'] as String,
        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              ...
              title: const Text('Phiếu Lương Của Tôi',
            ...
            error: (e, _) => SliverFillRemaining(
                child: Center(child: Column(
                  ...
                  children: [
                    ...
                    Text('Lỗi tải dữ liệu', style: TextStyle(
            ...
            data: (entries) {
                ...
                return SliverFillRemaining(
                  child: Center(child: Column(
                    ...
                    children: [
                      ...
                      Text('Chưa có phiếu lương nào',
                          style: TextStyle(color: Colors.grey[500], fontSize: 16,
                              fontWeight: FontWeight.w600)),
                      const SizedBox(height: 8),
                      Text('Kéo xuống để làm mới, hoặc liên hệ chủ quán',
                      ...
                      OutlinedButton.icon(
                        ...
                        style: OutlinedButton.styleFrom(
                          foregroundColor: const Color(0xFF1C2151),
                          side: const BorderSide(color: Color(0xFF1C2151)),
              ...
              return SliverPadding(
                ...
                sliver: SliverList(delegate: SliverChildBuilderDelegate(
                  (ctx, i) => _MyPayslipCard(entry: entries[i], onDisputed: () {
                    ref.invalidate(myPayslipsProvider);
                    // ‼️ Invalidate dispute provider của chính record này
                    // để card đổi ngay từ nút → badge "Đang chờ phản hồi"
                    ref.invalidate(_myDisputesProvider(entries[i].record.id));
                  }),
...
class _MyPayslipCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final disputes     = ref.watch(_myDisputesProvider(record.id)).value ?? [];
    final hasOpenDispute = disputes.any((d) => d['status'] == 'open');
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('${entry.periodFrom} → ${entry.periodTo}',
          ...
          Padding(
            ...
            child: Column(children: [
              // Stats row
              Row(children: [
                _StatItem(Icons.schedule_rounded, 'Tổng giờ',
                    '${record.totalHours.toStringAsFixed(1)}h', const Color(0xFF1565C0)),
                _StatItem(Icons.trending_up_rounded, 'Tăng ca',
                    '${record.overtimeHours.toStringAsFixed(1)}h', const Color(0xFF6A1B9A)),
                _StatItem(Icons.event_busy_rounded, 'Nghỉ',
                    '${record.absentDays}ngày', const Color(0xFFC62828)),
                _StatItem(Icons.alarm_rounded, 'Muộn',
                    '${record.lateCount}lần', const Color(0xFFE65100)),
              ]),
              const SizedBox(height: 12),
              ...
              Row(children: [
                const Text('Thực lĩnh',
                    style: TextStyle(color: Color(0xFF9E9085), fontSize: 13)),
                const Spacer(),
              ...
              if (!isPaid) ...[ const SizedBox(height: 12),
                if (hasOpenDispute)
                  // Đang có khiếu nại chờ xử lý → ẩn nút, hiện trạng thái
                  Container(
                    ...
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      Text('Đang chờ quản lý phản hồi',
                          style: TextStyle(color: Colors.orange.shade700,
                              fontWeight: FontWeight.w700, fontSize: 13)),
                    ]),
                  )
                else
                  // Không có open dispute → hiện nút (lần đầu hoặc sau khi resolved)
                  SizedBox(
                    width: double.infinity,
                    child: OutlinedButton.icon(
                      ...
                      label: Text(
                        hasAnyDispute ? 'Gửi khiếu nại mới' : 'Phản hồi / Khiếu nại',
  ...
  void _showDetail(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (_) => _PayslipDetailSheet(entry: entry),
    );
  }
...
class _PayslipDetailSheet extends ConsumerWidget {
            ...
            child: ListView(
              ...
              children: [
                ...
                _detailSection('Chi tiết khoản lương', [
                  ...
                  if (r.overtimePay > 0) _detailRow('Tăng ca', r.overtimePay, true),
                  if (r.bonusRevenue > 0) _detailRow('Thưởng doanh thu', r.bonusRevenue, true),
                  if (r.bonusManual > 0) _detailRow('Thưởng thủ công', r.bonusManual, true),
                  if (r.allowanceTotal > 0) _detailRow('Phụ cấp', r.allowanceTotal, true),
                  if (r.deductionLate > 0) _detailRow('Trừ đi muộn', -r.deductionLate, false),
                  if (r.deductionAbsent > 0) _detailRow('Trừ nghỉ', -r.deductionAbsent, false),
                  if (r.deductionManual > 0) _detailRow('Khấu trừ khác', -r.deductionManual, false),
                ]),
                const SizedBox(height: 16),

                // Attendance
                _detailSection('Chấm công', [
                  _infoRow('Tổng giờ làm', '${r.totalHours.toStringAsFixed(1)}h'),
                  _infoRow('Tăng ca', '${r.overtimeHours.toStringAsFixed(1)}h'),
                ...
                Row(children: [
                  const Icon(Icons.forum_rounded, size: 16, color: Color(0xFF1C2151)),
                  const SizedBox(width: 6),
                  const Text('Phản hồi của tôi',
                  ...
                  data: (disputes) {
                    if (disputes.isEmpty) {
                      return Container(
                        ...
                        child: Row(children: [
                          Icon(Icons.inbox_rounded, size: 20, color: Colors.grey[400]),
                          const SizedBox(width: 10),
                          Text('Chưa có khiếu nại nào',
                              style: TextStyle(color: Colors.grey[500], fontSize: 13)),
                        ]),
                      );
                    }
                    return Column(
                      children: disputes.map((d) => _DisputeReplyCard(d)).toList(),
                    );
                  },
                ),
  ...
  Widget _detailRow(String label, double amount, bool isPos) {
    final c = isPos ? Colors.green[700]! : Colors.red[700]!;
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: Row(children: [
        Expanded(child: Text(label, style: const TextStyle(fontSize: 13))),
        Text('${isPos ? '+' : ''}${_fmtMoney(amount)}',
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Icon(Icons.person_outline_rounded, size: 14, color: Color(0xFF9E9085)),
          const SizedBox(width: 6),
        ...
        if (reply != null && reply.isNotEmpty) ...[
          ...
          Container(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Phản hồi của quản lý',
                    style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                        color: Color(0xFF1565C0))),
                const SizedBox(height: 3),
                Text(reply, style: const TextStyle(
          ...
          const SizedBox(height: 8),
          Row(children: [
            Icon(Icons.access_time_rounded, size: 13, color: Colors.orange[400]),
            const SizedBox(width: 4),
            Text('Quản lý chưa phản hồi',
...
class _StatItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(child: Column(children: [
    ...
    Text(label, style: const TextStyle(
        color: Color(0xFF9E9085), fontSize: 9)),
  ]));
}
